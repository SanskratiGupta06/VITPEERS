...
export default function Messages() {
  ...
  return (
    <div className="vp-page-shell vp-page-shell-wide vp-messages-shell min-h-[calc(100vh-4rem)]">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-8"><div className="vp-page-heading mb-5"><span className="section-kicker">NETWORK / MESSAGES</span><p className="vp-page-code">PRIVATE CHANNEL</p></div>
          ...
          {loadingConvos ? (
            ...
            conversations.map((c) => (
              <Link
                key={c._id}
                to={`/messages/${c._id}`}
                className={`flex items-center gap-3 p-3 border-b border-line/60 vp-convo-row transition-colors ${
                  userId === c._id ? "vp-convo-row-active" : ""
                }`}
              >
        ...
        <div className="border border-line rounded-xl bg-white flex flex-col overflow-hidden">
          {!userId || !activeUser ? (
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-2 vp-surface-soft">
                {messages.map((m) => {
